...
manifest_file = "../../processed/bugtraq-database/manifest.txt"
...
def _is_mail_of_interest_by_text(text, filename, cve_mail_mappings):
    ...
    for cve_id in cve_id_pattern.findall(text):
        ...
        if f"bugtraq-database/{filename}" not in cve_mail_mappings[cve_id]:
            cve_mail_mappings[cve_id].append(f"bugtraq-database/{filename}")
